...
解壓後得到一個 `NHITW_clinic_reader` 資料夾：`dist`（擴充套件）＋`native-host`（共享資料夾主機）＋`README.html`（**給醫師的完整安裝與自行更新教學**，雙擊即可閱讀）。裝在櫃檯（讀卡查雲端）的電腦。
...
1. 下載上面的完整安裝包，解壓出的 `NHITW_clinic_reader` 資料夾放到**固定位置**（例如 `C:\NHITW_clinic_reader`）— 之後不能刪、不能搬家
2. 打開 Chrome，網址列輸入 `chrome://extensions`（Edge 輸入 `edge://extensions`）
3. 打開右上角的「**開發人員模式**」
4. 點「**載入未封裝項目**」→ 選 `NHITW_clinic_reader` 裡的 **`dist`** 資料夾
...
1. 進入 `NHITW_clinic_reader` 裡的 **`native-host`** 資料夾
2. 雙擊 **`install.bat`**
3. 黑色視窗會問兩個問題：
...
1. 下載新的完整安裝包，用解壓出的 `NHITW_clinic_reader` 內容**覆蓋原本的安裝位置**
2. 回到 `chrome://extensions` 按該套件的「重新整理 ↻」圖示
3. 若公告有提到「主機更新」，再進 `native-host` 重跑一次 `install.bat`（很少發生）

> 完整的圖文教學（含從舊版包遷移的注意事項）就在包內的 `README.html`，醫師可自行照做。
