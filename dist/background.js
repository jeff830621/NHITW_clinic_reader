...
\u4F9D\u64DA\uFF1ACr=${n} mg/dL @ ${t.date}`;return`<span class="ckd-badge ckd-watch" title="${l(d)}">\u{1FAD8} CKD \u5F85\u78BA\u8A8D (stage 2)</span>`}var it={NSAID:["M01AA","M01AB","M01AC","M01AE","M01AG","M01AH"],ACEI:["C09AA","C09BA","C09BB","C09BX"],ARB:["C09CA","C09DA","C09DB","C09DX"],STATIN:["C10AA","C10BA","C10BX"],SGLT2:["A10BK","A10BD15","A10BD16","A10BD19","A10BD20","A10BD21","A10BD25","A10BD27","A10BD29","A10BD30"],GLP1:["A10BJ","A10AE54","A10AE56"],\u6297\u51DD:["B01A"]},st={red:["\u6297\u51DD","NSAID"],orange:["ARB","ACEI","STATIN"],green:["SGLT2","GLP1"]},ct={red:{bg:"#fde8e8",border:"#e53935",text:"#b71c1c"},orange:{bg:"#fff3e0",border:"#fb8c00",text:"#e65100"},green:{bg:"#e8f5e9",border:"#43a047",text:"#1b5e20"}};function pe(e){if(!e)return null;for(let[a,t]of Object.entries(it))if(t.some(n=>n.length===7?e===n:e.startsWith(n)))return a;return null}function lt(e){for(let[a,t]of Object.entries(st))if(t.includes(e))return a;return null}function B(e,a){if(!e)return!1;let t=new Date(e);return isNaN(t.getTime())&&e.includes("/")&&(t=new Date(e.replace(/\//g,"-"))),isNaN(t.getTime())?!1:Date.now()-t.getTime()<=a*864e5}function dt(e,a){if(!e||e.length===0)return'<p class="empty">\u7121\u897F\u85E5\u7D00\u9304</p>';let t=a||100,n=[];for(let o of e){let c=o.PER_DATE||o.drug_date||"";if(!B(c,t))continue;let d=o.ATC_CODE||o.drug_atc7_code||"",u=pe(d);if(!u)continue;let g=lt(u);g&&n.push({name:o.MED_DESC||o.MED_ITEM||o.drug_ename||"",generic:o.GENERIC_NAME||o.drug_ing_name||"",date:k(c),hosp:O(o.HOSP_NAME||o.hosp),freq:o.FREQ_DESC||o.drug_fre||"",medDays:o.MED_DAYS||o.day||"",drugLeft:o.DRUG_LEFT||o.drug_left||"",groupName:u,colorName:g})}if(n.length===0)return'<p class="empty">\u7121\u95DC\u6CE8\u897F\u85E5\u7D00\u9304</p>';let s=["red","orange","green"],r={};for(let o of n){r[o.colorName]||(r[o.colorName]={}),r[o.colorName][o.groupName]||(r[o.colorName][o.groupName]={});let c=o.name;r[o.colorName][o.groupName][c]||(r[o.colorName][o.groupName][c]={...o,prescriptions:[]}),r[o.colorName][o.groupName][c].prescriptions.push({date:o.date,hosp:o.hosp,days:o.medDays,drugLeft:o.drugLeft})}let i="";for(let o of s){if(!r[o])continue;let c=ct[o];for(let[d,u]of Object.entries(r[o]))for(let g of Object.values(u)){g.prescriptions.sort((m,h)=>(h.date||"").localeCompare(m.date||""));let p=g.prescriptions.slice(0,3).map(m=>`<span class="med-pres">${l(M(m.date))} ${l(m.hosp)}${m.drugLeft&&m.drugLeft!=="0"?` <span class="drug-left">\u9918${m.drugLeft}\u5929</span>`:""}</span>`).join(" ");i+=`<tr>
  ...
  <div class="tracking-note">${t} \u5929\u5167</div>`}function ut(e,a){if(!e||e.length===0)return'<p class="empty">\u7121\u897F\u85E5\u7D00\u9304</p>';let t=a||100,n=[];for(let o of e){let c=o.PER_DATE||o.drug_date||"";if(!B(c,t))continue;let d=o.ATC_CODE||o.drug_atc7_code||"";pe(d)||n.push({name:o.MED_DESC||o.MED_ITEM||o.drug_ename||"",generic:o.GENERIC_NAME||o.drug_ing_name||"",date:k(c),hosp:O(o.HOSP_NAME||o.hosp),icd:o.ICD_CODE||o.icd_code||"",icdName:o.ICD_NAME||o.icd_cname||"",freq:o.FREQ_DESC||o.drug_fre||"",medDays:o.MED_DAYS||o.day||"",drugLeft:o.DRUG_LEFT||o.drug_left||""})}if(n.length===0)return'<p class="empty">\u7121\u5176\u4ED6\u897F\u85E5\u7D00\u9304</p>';let s={};for(let o of n){let c=`${o.date}|${o.hosp}`;s[c]||(s[c]={date:o.date,hosp:o.hosp,icd:o.icd,icdName:o.icdName,meds:[]}),s[c].meds.push(o)}let r=Object.values(s).sort((o,c)=>(c.date||"").localeCompare(o.date||"")),i="";for(let o of r){i+=`<div class="med-group-header">${l(M(o.date))} ${l(o.hosp)}`,o.icd&&(i+=` <span class="diag-code">${l(o.icd)}</span>`),i+="</div>";for(let c of o.meds){let d=[c.freq,c.medDays?c.medDays+"\u5929":"",c.drugLeft&&c.drugLeft!=="0"?`\u9918${c.drugLeft}\u5929`:""].filter(Boolean).join(" ");i+=`<div class="med-item">${l(c.name)} <span class="med-detail">${l(d)}</span></div>`}}return i+=`<div class="tracking-note">${t} \u5929\u5167</div>`,i}function pt(e){if(!e||e.length===0)return'<p class="empty">\u7121\u4E2D\u85E5\u7D00\u9304</p>';let a={};for(let n of e){let s=k(n.func_date||""),r=O(n.hosp),i=`${s}|${r}`;a[i]||(a[i]={date:s,hosp:r,icd:n.icd_code||"",icdName:n.icd_cname||"",meds:[]}),a[i].meds.push(n)}let t="";for(let n of Object.values(a)){t+=`<div class="med-group-header">${l(M(n.date))} ${l(n.hosp)}`,n.icd&&(t+=` <span class="diag-code">${l(n.icd)}</span>`),t+="</div>";let s=new Map;for(let r of n.meds){let i=r.drug_perscrn_name||r.cdrug_name||"",o=parseFloat(r.order_qty)||0,c=r.drug_fre||"",d=r.day||"",u=`${i}|${c}|${d}`,g=s.get(u);g?o>g.qty?(g.alts.push(g.raw),g.qty=o,g.raw=r.order_qty||""):String(r.order_qty||"")!==g.raw&&g.alts.push(r.order_qty||""):s.set(u,{name:i,qty:o,freq:c,days:d,raw:r.order_qty||"",alts:[]})}for(let r of s.values()){let i=r.alts.length?` title="\u540C\u7B46\u8655\u65B9\u53E6\u5B58 qty=${l(r.alts.join(", "))}"`:"",o=r.alts.length?' <span class="med-dup">\u26A0</span>':"";t+=`<div class="med-item"${i}>${l(r.name)} <span class="med-detail">${l(r.raw)} ${l(r.freq)} ${r.days?r.days+"\u5929":""}${o}</span></div>`}}return t}var ft=new Set(["33085B","33084B","33072B","33070B","19009C","19001C","18006C","28016C"]);function gt(e){if(!e||e.length===0)return'<p class="empty">\u7121\u5F71\u50CF\u8CC7\u6599</p>';let a=180,t=e.filter(i=>{let o=i.real_inspect_date||i.case_time||i.recipe_date||"";if(!B(o,a))return!1;let c=i.order_code||"";return ft.has(c)});if(t.length===0)return`<p class="empty">${a}\u5929\u5167\u7121\u95DC\u6CE8\u7684\u5F71\u50CF\u6AA2\u67E5</p>`;let n=new Set,s=[];for(let i of t){let o=k(i.real_inspect_date||i.case_time||i.recipe_date||""),c=i.order_name||"",d=i.order_code||"",u=`${o}|${c}|${d}`;n.has(u)||(n.add(u),s.push(i))}s.sort((i,o)=>{let c=k(i.real_inspect_date||i.case_time||i.recipe_date||"");return k(o.real_inspect_date||o.case_time||o.recipe_date||"").localeCompare(c)});let r="";for(let i of s){let o=M(i.real_inspect_date||i.case_time||i.recipe_date||""),c=i.order_name||"";c=c.replace(/[[\]]/g,"").replace(/;/g," ").trim();let d=O(i.hosp),u=i.inspect_result||"";r+='<div class="imaging-item">',r+=`<div class="imaging-name">${l(c)}</div>`,r+=`<div class="imaging-meta">${l(o)} ${l(d)}</div>`,u&&(r+=`<div class="imaging-result">${l(u)}</div>`),r+="</div>"}return r+=`<div class="tracking-note">${a} \u5929\u5167</div>`,r}function mt(e){if(!e||e.length===0)return'<p class="empty">\u7121\u904E\u654F\u7D00\u9304</p>';let a=e.filter(n=>{let s=n.drug_name||"";return s&&!s.includes("\u672A\u8A18\u9304")&&s!=="NP"&&s!=="N.P"&&s!=="N.P."&&!s.includes("\u672A\u904E\u654F")});if(a.length===0)return'<p class="empty">\u7121\u904E\u654F\u7D00\u9304</p>';let t="";for(let n of a){let s=n.drug_name||"",r=(n.sympton_name||"").replace(/;/g,", ");t+=`<div class="allergy-item"><strong>${l(s)}</strong>${r?` \u2014 ${l(r)}`:""}</div>`}return t}function bt(e){if(!e||e.length===0)return"";let a="";for(let t of e){let n=M(t.exe_s_date||""),s=O(t.hosp),r=t.icd_cname||t.icd_code||"";a+=`<div class="record-item">${l(n)} ${l(s)} \u2014 ${l(r)}</div>`}return a}function ht(e){if(!e||e.length===0)return"";let a="";for(let t of e){let n=M(t.in_date||""),s=M(t.out_date||""),r=O(t.hosp),i=t.icd_cname||t.icd_code||"";a+=`<div class="record-item">${l(n)}~${l(s)} ${l(r)} \u2014 ${l(i)}</div>`}return a}function Ct(e){return e?Array.isArray(e.rObject)&&e.rObject[0]?e.rObject[0]:e.originalData?.robject?e.originalData.robject:e.result_data?e:null:null}function yt(e){return e?Array.isArray(e.rObject)&&e.rObject[0]?e.rObject[0]:e.originalData?.robject?e.originalData.robject:e.colorectal||e.oralMucosa||e.mammography||e.papSmears||e.lungCancer?e:null:null}function vt(e){return e?Array.isArray(e.rObject)&&e.rObject[0]?e.rObject[0]:e.originalData?.robject?e.originalData.robject:e.result_data||e.med_data?e:null:null}function xt(e){let a=Ct(e);if(!a||!Array.isArray(a.result_data)||a.result_data.length===0)return"";let t=a.result_data[0],n=s=>s==null||s===""?"\u2014":l(String(s));return`<div class="hc-title">${n(t.title||"\u6700\u8FD1\u4E00\u6B21")}</div>
...
</html>`}var C={medicationData:null,labData:null,chinesemedData:null,acupunctureData:null,imagingData:null,allergyData:null,surgeryData:null,dischargeData:null,medDaysData:null,patientSummaryData:null,adultHealthCheckData:null,cancerScreeningData:null,hbcvData:null,token:null,currentUserSession:null,patientName:null,patientIdFromToken:null},G=-1,fe=new Set(["token","currentUserSession","patientName","patientIdFromToken"]);function P(){for(let e of Object.keys(C))fe.has(e)||(C[e]=null)}function St(e){let a=e||new Date,t=a.getHours()*60+a.getMinutes();return t<780?"\u65E9\u8A3A":t<1070?"\u5348\u8A3A":"\u665A\u8A3A"}var K=!1;function ge(){K||(K=!0,chrome.alarms.create("htmlExport",{delayInMinutes:.1}))}chrome.alarms.onAlarm.addListener(e=>{e.name==="htmlExport"&&(K=!1,At().catch(a=>{console.warn("[NHITW Clinic] Export alarm handler error:",a.message)}))});async function At(){try{if(!((await chrome.storage.sync.get("sharedFolder")).sharedFolder||{}).enabled)return;let t={age:null,sex:"",birthday:""},n=null,s=G;if(s>0)try{n=await chrome.tabs.sendMessage(s,{action:"getPatientInfo"}),console.log(`[NHITW Clinic] Fresh patient info from tracked tab ${s}:`,n)}catch(h){console.warn(`[NHITW Clinic] Tracked tab ${s} unreachable: ${h.message}, falling back`),s=-1}if(!n)try{let h=await chrome.tabs.query({url:"https://medcloud2.nhi.gov.tw/*"}),_=h.find(w=>w.active)||h[0];_&&(n=await chrome.tabs.sendMessage(_.id,{action:"getPatientInfo"}),console.log(`[NHITW Clinic] Fresh patient info from active fallback tab ${_.id}:`,n))}catch(h){console.warn("[NHITW Clinic] Fallback getPatientInfo also failed:",h.message)}if(n?.id){let h=n.id!==C.patientIdFromToken;C.patientIdFromToken=n.id,n.name?C.patientName=n.name:(h||!C.patientName)&&(C.patientName=n.id),t={age:n.age??null,sex:n.sex||"",birthday:n.birthday||""}}let r=C.patientIdFromToken,i=C.patientName;if(!r){let h=C.currentUserSession;if(!h){console.log("[NHITW Clinic] No session data, skipping export");return}r=h.startsWith("patient_")?h.replace("patient_",""):h}i||(i=r),console.log(`[NHITW Clinic] Export: ID=${T(r,4,3)}, Name=${T(i,1,1)}`);let o={};for(let[h,_]of Object.entries(C)){if(fe.has(h))continue;let w=_?.rObject;o[h]=Array.isArray(w)?w.length:_==null?0:"present"}t._identityProbe={generatedAt:new Date().toISOString(),resolvedName:T(i,1,1),resolvedId:T(r,4,3),nameSource:i===r?"fallback_to_id":C.patientName?"cache":"fresh",activeTabId:G,dataAtExport:o,cached:{name:T(C.patientName||"",1,1),id:T(C.patientIdFromToken||"",4,3),session:C.currentUserSession||null},freshFromTab:n?{name:T(n.name||"",1,1),id:T(n.id||"",4,3),hasAge:n.age!=null,hasSex:!!n.sex,hasBirthday:!!n.birthday,_debug:n._debug||null}:null};let c={};for(let[h,_]of Object.entries(C))h!=="token"&&h!=="currentUserSession"&&_&&(c[h]=_);let d=X(i,r,c,t),u=ee(i),g=St(new Date),p=Math.round(new Blob([d]).size/1024),m=p;if(console.log(`[NHITW Clinic] Generating HTML: ${u} (${m}KB, ${Object.keys(c).length} data types, session=${g})`),m>900&&(d=wt(d),m=Math.round(new Blob([d]).size/1024),m<=900&&console.warn(`[NHITW Clinic] HTML trimmed (${p}KB \u2192 ${m}KB) by dropping debug comment`)),m>900){console.warn(`[NHITW Clinic] HTML too large (${m}KB) \u2014 writing oversize stub instead`);try{chrome.action.setBadgeText({text:"\u26A0"}),chrome.action.setBadgeBackgroundColor({color:"#c62828"})}catch{}d=kt(i,r,p),await j(u,d,void 0,g);return}await j(u,d,void 0,g),console.log(`[NHITW Clinic] HTML report saved: ${g}/${u}`)}catch(e){console.warn("[NHITW Clinic] Auto-export failed (non-blocking):",e.message)}}function wt(e){return e.replace(/<!-- NHITW-DEBUG-START[\s\S]*?NHITW-DEBUG-END -->\n?/g,"")}function T(e,a=1,t=1){if(e==null)return"";let n=String(e);return n.length<=a+t?n:n.slice(0,a)+"*".repeat(Math.min(3,n.length-a-t))+n.slice(-t)}function kt(e,a,t){let n=r=>String(r??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),s=new Date().toLocaleString("zh-TW");return`<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><title>${n(e)} \u2014 \u5831\u544A\u904E\u5927</title>
